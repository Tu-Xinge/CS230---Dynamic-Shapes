     ...
     * @param y    the y-coordinate of the mouse position */
	  ...
	  protected void createNewShape(int x, int y) {
        ...
        switch (currentShapeType) {
            case RECTANGLE: {
            	s = new RectangleShape(x, y,currentWidth,currentHeight,marginWidth,marginHeight,currentBorderColor,currentFillColor,currentPathType,currentText);
            	s.setParent(root);
        		treeModelAdapter.addToRoot(s);
//        		shapes.add(s);
                tableModelAdapter.fireTableDataChanged();
                break;
			} case XRECTANGLE: {
				s = new XRectangleShape(x, y,currentWidth,currentHeight,marginWidth,marginHeight,currentBorderColor,currentFillColor,currentPathType,currentText );
				s.setParent(root);
				treeModelAdapter.addToRoot(s);
//				shapes.add(s);
			   	tableModelAdapter.fireTableDataChanged();
		   		...
		   		s = new OvalShape(x,y,currentWidth,currentHeight,marginWidth,marginHeight,currentBorderColor,currentFillColor,currentPathType,currentText);
		   		s.setParent(root);
				treeModelAdapter.addToRoot(s);
//				shapes.add(s);
				tableModelAdapter.fireTableDataChanged();
				break;
		   		...
		   		s = new SquareShape(x,y,Math.min(currentWidth,currentHeight),marginWidth,marginHeight,currentBorderColor,currentFillColor,currentPathType,currentText);
				s.setParent(root);
		   		treeModelAdapter.addToRoot(s);
				tableModelAdapter.fireTableDataChanged();
				break;
			} case NESTED: {
				s = new NestedShape(x,y,currentWidth,currentHeight,marginWidth,marginHeight,currentBorderColor,currentFillColor,currentPathType,currentText);
				s.setParent(root);
				treeModelAdapter.addToRoot(s);
//				shapes.add(s);
				tableModelAdapter.fireTableDataChanged();
	...
	public boolean addShape(TreePath selectedPath){
		boolean a = false;
		a = treeModelAdapter.addNode(selectedPath, currentShapeType);
		tableModelAdapter.fireTableDataChanged();
		return a;
	}
	public boolean removeShape(TreePath selectedPath){
		boolean a = false;
		a = treeModelAdapter.removeNodeFromParent(selectedPath);
		tableModelAdapter.fireTableDataChanged();
		return a;
	}
}
